...
os::apple::socket_osx::socket_osx(os::create_only_t, const std::string name)
{
	...
	if (mkfifo(name_req.c_str(), S_IRUSR | S_IWUSR) < 0)
		throw std::exception((const std::exception &)"Could not create request pipe");

	remove(name_rep.c_str());
	if (mkfifo(name_rep.c_str(), S_IRUSR | S_IWUSR) < 0)
		throw std::exception((const std::exception &)"Could not create reply pipe");
...
os::error os::apple::socket_osx::accept(std::shared_ptr<os::async_op> &op, os::async_op_cb_t cb)
{
	...
	ar->set_callback(cb);
	ar->set_system_callback(std::bind(&os::apple::socket_osx::handle_accept_callback, this, std::placeholders::_1, std::placeholders::_2));
	ar->set_sem(NULL);
	connected = true;
